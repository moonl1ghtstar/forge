/*
 * forge-asm.c - Assembler implementation for Forge
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int forge_assemble_text(const char *asm_text, const char *obj_path) {
    ...
    if (!prog || prog->had_error) {
        fprintf(stderr, "forge-asm: assembly parsing failed.\n");
    ...
    for (i = 0; i < prog->count; i++) {
        ...
        if (s->kind == ASM_STMT_SECTION) {
            ...
            *off_ptr += size;
            ...
            if (sz < 0) {
                fprintf(stderr, "forge-asm: line %d: error measuring instruction '%s'\n",
                        s->line, s->mnemonic);
                goto error_cleanup;
            }
            text_offset += sz;
    ...
    for (i = 0; i < prog->count; i++) {
        ...
        if (s->kind == ASM_STMT_SECTION) {
            ...
            if (sz < 0) {
                fprintf(stderr, "forge-asm: line %d: failed to encode instruction '%s'\n",
                        s->line, s->mnemonic);
...
int forge_assemble_file(const char *asm_path, const char *obj_path) {
    ...
    if (!content) {
        fprintf(stderr, "forge-asm: cannot read file '%s'\n", asm_path);
        return 1;
    }
    int rc = forge_assemble_text(content, obj_path);
    free(content);
    return rc;
}
