...
int coff_write_object(const char *output_path,
    ...
    if (text_sec && text_sec->size > 0) text_idx = ++num_sections;
    if (data_sec && data_sec->size > 0) data_idx = ++num_sections;
    if (bss_size > 0) bss_idx = ++num_sections;
    ...
    if (text_idx > 0) {
        ...
        if (text_reloc_count > 0) {
            ...
            for (i = 0; i < text_reloc_count; i++) {
                ...
                if (sym_idx < 0) {
                    fprintf(stderr, "forge-coff: error: relocation to unresolved symbol '%s'\n",
    ...
    for (i = 0; i < symbol_count; i++) {
        ...
        /* Section mapping */
        if (s->section_num == 1)      se->SectionNumber = text_idx;
        else if (s->section_num == 2) se->SectionNumber = data_idx;
        else if (s->section_num == 3) se->SectionNumber = bss_idx;
        else                          se->SectionNumber = 0; /* UNDEF (extern) */
