...
static int fits_imm32(long v) { return v >= -2147483648L && v <= 2147483647L; }

/* Emit one byte into buf[n], return 0 on overflow */
#define EMIT(b) do { if (n >= buf_size) return -1; buf[n++] = (uint8_t)(b); } while(0)
...
#define EMIT_REX(W,R,X,B) do { \
    uint8_t rex_ = REX(W,R,X,B); \
    if (rex_ != 0x40) EMIT(rex_); \
} while(0)
    ...
    if (dst->kind == ASMOP_REG && src->kind == ASMOP_IMM) {
        ...
        if (W) {
            ...
            if (fits_imm32(val)) {
                ...
                EMIT(v32 & 0xFF); EMIT((v32>>8)&0xFF);
                EMIT((v32>>16)&0xFF); EMIT((v32>>24)&0xFF);
            } else {
                /* Full 64-bit: REX.W B8+rd imm64 */
                EMIT(REX(1,0,0,REGHI(dst->reg_idx)));
                EMIT(0xB8 | REGLO(dst->reg_idx));
                uint64_t v64 = (uint64_t)val;
                int k; for (k = 0; k < 8; k++) EMIT((v64>>(k*8))&0xFF);
            }
        } else {
            /* 32-bit MOV: B8+rd imm32 */
            if (REGHI(dst->reg_idx)) EMIT(REX(0,0,0,1));
            EMIT(0xB8 | REGLO(dst->reg_idx));
...
int x86_encode(X86EncodeCtx *ctx, const AsmStatement *stmt,
               uint8_t *buf, int buf_size) {
    if (!stmt || stmt->kind != ASM_STMT_INSTR) return -1;
    const char *m = stmt->mnemonic;

    if (strcmp(m, "mov")  == 0) return encode_mov(ctx, stmt, buf, buf_size);
    if (strcmp(m, "lea")  == 0) return encode_lea(ctx, stmt, buf, buf_size);

    if (strcmp(m, "add")  == 0) return encode_alu(stmt, buf, buf_size, 0, 0x03);
    if (strcmp(m, "or")   == 0) return encode_alu(stmt, buf, buf_size, 1, 0x0B);
    if (strcmp(m, "and")  == 0) return encode_alu(stmt, buf, buf_size, 4, 0x23);
    if (strcmp(m, "sub")  == 0) return encode_alu(stmt, buf, buf_size, 5, 0x2B);
    if (strcmp(m, "xor")  == 0) return encode_alu(stmt, buf, buf_size, 6, 0x33);
    if (strcmp(m, "cmp")  == 0) return encode_alu(stmt, buf, buf_size, 7, 0x3B);
    ...
    if (strcmp(m, "cdq") == 0) {
        int n = 0; EMIT(0x99); return n;
    }
    if (strcmp(m, "cqo") == 0) {
        int n = 0; EMIT(0x48); EMIT(0x99); return n;
    }
    if (strcmp(m, "nop") == 0) {
        int n = 0; EMIT(0x90); return n;
    }

    if (strcmp(m, "call") == 0) return encode_call(ctx, stmt, buf, buf_size);

    /* Unconditional JMP */
    if (strcmp(m, "jmp") == 0)  return encode_jmp(ctx, stmt, buf, buf_size, 0xE9);

    /* Conditional jumps (near form: 0F 8x rel32) */
    if (strcmp(m,"jo")  ==0) return encode_cjmp(ctx,stmt,buf,buf_size,0x00);
    if (strcmp(m,"jno") ==0) return encode_cjmp(ctx,stmt,buf,buf_size,0x01);
    ...
    if (strcmp(m,"js")  ==0) return encode_cjmp(ctx,stmt,buf,buf_size,0x08);
    if (strcmp(m,"jns") ==0) return encode_cjmp(ctx,stmt,buf,buf_size,0x09);
    ...
    /* SETcc */
    if (strcmp(m,"seto")  ==0) return encode_setcc(stmt,buf,buf_size,0x00);
    ...
    if (strcmp(m,"setbe") ==0) return encode_setcc(stmt,buf,buf_size,0x06);
    if (strcmp(m,"seta")  ==0) return encode_setcc(stmt,buf,buf_size,0x07);
    if (strcmp(m,"sets")  ==0) return encode_setcc(stmt,buf,buf_size,0x08);
...
int x86_measure(const AsmStatement *stmt) {
    if (!stmt || stmt->kind != ASM_STMT_INSTR) return 0;
    const char *m = stmt->mnemonic;
    ...
    int i; for (i = 0; i < dummy.reloc_count; i++) free(dummy.relocs[i].symbol);
    free(dummy.relocs);
    if (r < 0) {
        /* Fallback: near jump is 5 (jmp) or 6 (jcc), call=5 */
        if (strncmp(m,"j",1)==0) return 6;
