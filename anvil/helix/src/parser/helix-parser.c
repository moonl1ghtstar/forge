/*
 * parser.c - Recursive descent parser for the Anvil compiler
 *
...
/* Report a parse error to stderr in the standard Anvil format */
static void parse_error(Parser *p, const char *msg) {
    ...
    if (p->current.type == TOKEN_ERROR) {
        ...
        anv_report_error(SEV_ERROR, err_code, err_line, err_col, NULL, NULL, "%s", p->current.lexeme ? p->current.lexeme : msg);
    } else {
        if (strstr(msg, "expected ')'"))
            ...
            err_code = "E206";
        else if (strstr(msg, "only function declarations"))
            err_code = "E207";
        else if (strstr(msg, "expected an identifier after 'function'"))
            err_code = "E202";
        else if (strstr(msg, "expected identifier") || strstr(msg, "expected function name") || strstr(msg, "expected parameter name") || strstr(msg, "expected variable name") || strstr(msg, "expected struct type name") || strstr(msg, "expected field name") || strstr(msg, "expected module name"))
            err_code = "E202";
        ...
        anv_report_error(SEV_ERROR, err_code, err_line, err_col, NULL, NULL, "%s", formatted_msg);
    }
    p->had_error = 1;
}
...
static char *read_file(const char *path, int is_system) {
    ...
    if (is_system) {
#ifdef _WIN32
        // Only search in the anv.exe directory for system modules
...
static int load_module(Parser *p, ASTNode *prog, const char *module_name, int is_system, char **select_funcs, int select_count) {
    ...
    if (is_system) {
        ...
        if (!config_content) {
            anv_report_error(SEV_ERROR, "E601", p->current.line, p->current.col, "note", "config.json missing or unreadable", "failed to import module '%s'", module_name);
            return 1;
        }
        ...
        if (select_count > 0) {
            ...
            for (i = 0; i < select_count; i++) {
                ...
                if (!found) {
                    anv_report_error(SEV_ERROR, "E602", p->current.line, p->current.col, NULL, NULL, "symbol '%s' was not found in module '%s'", select_funcs[i], module_name);
                    if (source_dir)
                        free(source_dir);
                    free(config_content);
                    if (module_funcs) {
        ...
        for (i = 0; i < load_count; i++) {
            ...
            if (!func_source) {
                anv_report_error(SEV_ERROR, "E601", p->current.line, p->current.col, NULL, NULL, "failed to import module '%s': cannot load function '%s'", module_name, func_name);
                if (source_dir)
                    free(source_dir);
                free(config_content);
                if (module_funcs) {
            ...
            if (sub.had_error) {
                anv_report_error(SEV_ERROR, "E601", p->current.line, p->current.col, NULL, NULL, "failed to import module '%s': function '%s' has parse errors", module_name, func_name);
        ...
        if (!loading_stack_push(module_name)) {
            anv_report_error(SEV_ERROR, "E603", p->current.line, p->current.col, NULL, NULL, "circular module dependency detected: '%s' imports itself transitively", module_name);
            return 1;
        }
        ...
        if (!project_source) {
            anv_report_error(SEV_ERROR, "E601", p->current.line, p->current.col, NULL, NULL, "failed to import module '%s'", module_name);
            loading_stack_pop();
            return 1;
        }
        ...
        if (sub.had_error) {
            anv_report_error(SEV_ERROR, "E601", p->current.line, p->current.col, NULL, NULL, "failed to import module '%s': module has parse errors", module_name);
...
static ASTNode *parse_var_decl(Parser *p) {
    ...
    if (!check(p, TOKEN_SEMICOLON)) {
        ...
        anv_report_error(SEV_ERROR, "E204", err_line, err_col, NULL, NULL,
