/*
 * ast.c - AST node creation and destruction for the Anvil compiler
 *
...
ASTNode *ast_new(ASTNodeType type, int line) {
    ...
    if (!node) {
        anv_report_error(SEV_ERROR, "E901", 0, 0, NULL, NULL, "internal compiler error: memory allocation failed");
        exit(1);
    }
    node->type = type;
    node->line = line;
