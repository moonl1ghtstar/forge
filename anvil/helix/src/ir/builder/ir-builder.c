/*
 * ir-builder.c - Lower Anvil AST into IRProgram.
 */
...
static IRValue lower_field_access(IRBuilder *b, ASTNode *node) {
    ...
    IRValue out;
    if (!path) {
        fprintf(stderr, "Anvil IR error: line %d: invalid field access.\n", node->line);
...
static void lower_struct_init(IRBuilder *b, ASTNode *node) {
    ...
    int i;
    if (!info) {
        fprintf(stderr, "Anvil IR error: line %d: unknown struct '%s'\n", node->line, node->as.struct_init.type_name);
        return;
    }
    ...
    for (i = 0; i < info->field_count; i++) {
        ...
        if (idx < 0 || idx >= info->field_count) {
            fprintf(stderr, "Anvil IR error: line %d: invalid field initializer.\n", node->line);
            continue;
        }
        if (!value_node)
            continue;
...
static IRValue lower_expr(IRBuilder *b, ASTNode *node) {
    ...
    case AST_ASSIGN:
        ...
        if (idx < 0) {
            fprintf(stderr, "Anvil IR error: line %d: unknown variable '%s'\n", node->line, node->as.assign.name);
            return right;
        }
...
static void lower_stmt(IRBuilder *b, ASTNode *node) {
    ...
    case AST_BREAK:
        if (b->loop_depth > 0)
            ...
            fprintf(stderr, "Anvil IR error: line %d: break() outside loop\n", node->line);
        break;
    case AST_PASS:
        if (b->loop_depth > 0)
            terminate_jump(b, b->loop_stack[b->loop_depth - 1].continue_block);
        else
            fprintf(stderr, "Anvil IR error: line %d: pass() outside loop\n", node->line);
        break;
    case AST_BLOCK:
        lower_block(b, node);
        break;
