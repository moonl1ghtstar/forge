/*
 * c-parser.c - Recursive descent parser for Anvil C frontend
 */
...
#include <string.h>
#include "c-parser.h"
#include "../../../helix/src/errors/helix-errors.h"

static void c_parse_error(CParser *p, const char *msg) {
    const char *err_code = "E201";
    if (p->current.type == C_TOK_ERROR) {
        err_code = "E101";
        anv_report_error(SEV_ERROR, err_code, p->current.line, p->current.col, NULL, NULL, "%s", p->current.lexeme ? p->current.lexeme : msg);
    } else {
        if (strstr(msg, "expected ')'"))
            ...
            err_code = "E206";
        else if (strstr(msg, "expected identifier") || strstr(msg, "expected function name") || strstr(msg, "expected parameter name") || strstr(msg, "expected variable name"))
            err_code = "E202";
        ...
        anv_report_error(SEV_ERROR, err_code, p->current.line, p->current.col, NULL, NULL, "%s", formatted_msg);
    }
    p->had_error = 1;
}
