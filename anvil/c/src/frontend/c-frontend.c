/*
 * c-frontend.c - Frontend pipeline for Anvil C subset
 */
...
static char *read_file(const char *path) {
    ...
    if (!buf) {
        anv_report_error(SEV_ERROR, "E901", 0, 0, NULL, NULL, "internal compiler error: memory allocation failed reading '%s'", path);
        fclose(f);
        return NULL;
    }
...
int compile_c(const char *source_path, const char *output_path) {
    ...
    if (result != 0) {
        fprintf(stderr, "Anvil error: C parsing failed. Fix the errors above, then run again.\n");
        return 1;
    }
