/*
 * x86-encode.c - x86-64 instruction encoder for Anvil ASM
 *
 * Encoding strategy:
 ...
 *
 * Supported mnemonics (superset of Anvil's codegen output):
...
static int encode_lea(X86EncodeCtx *ctx, const AsmStatement *stmt,
    ...
    const AsmOperand *src = &stmt->operands[1];
    if (dst->kind != ASMOP_REG || src->kind != ASMOP_MEM) {
        fprintf(stderr, "anv-asm: line %d: LEA requires reg, [mem]\n", stmt->line);
        return -1;
    }
    int W = (dst->reg_size == 64);
    if (src->mem.kind == MEMKIND_RIP_REL) {
...
static int encode_alu(const AsmStatement *stmt, uint8_t *buf, int buf_size,
    ...
    fprintf(stderr, "anv-asm: line %d: unsupported ALU form for '%s'\n",
...
static int encode_movzx(const AsmStatement *stmt,
    ...
    if (dst->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: MOVZX requires reg dst\n", stmt->line);
        return -1;
    }
    int W = (dst->reg_size == 64);
    if (src->kind == ASMOP_REG) {
        ...
        fprintf(stderr, "anv-asm: line %d: MOVZX: unsupported operand form\n", stmt->line);
        return -1;
    }
    return n;
}
...
static int encode_movsx(const AsmStatement *stmt,
    ...
    const AsmOperand *src = &stmt->operands[1];
    if (dst->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: MOVSX requires reg dst\n", stmt->line);
        return -1;
    }
    int W = (dst->reg_size == 64);
    if (src->kind == ASMOP_REG) {
        ...
        fprintf(stderr, "anv-asm: line %d: MOVSX: unsupported operand form\n", stmt->line);
        return -1;
    }
    return n;
}
...
static int encode_test(const AsmStatement *stmt,
    ...
    fprintf(stderr, "anv-asm: line %d: TEST requires reg,reg or reg,imm\n", stmt->line);
    return -1;
}
...
static int encode_push(const AsmStatement *stmt,
    ...
    fprintf(stderr, "anv-asm: line %d: PUSH requires register or immediate\n", stmt->line);
    return -1;
}
...
static int encode_pop(const AsmStatement *stmt,
    ...
    const AsmOperand *op = &stmt->operands[0];
    if (op->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: POP requires register\n", stmt->line);
        return -1;
    }
    if (REGHI(op->reg_idx))
        EMIT(REX(0, 0, 0, 1));
...
static int encode_call(X86EncodeCtx *ctx, const AsmStatement *stmt,
    ...
    fprintf(stderr, "anv-asm: line %d: CALL requires label or register\n", stmt->line);
    return -1;
}
...
static int encode_imul(const AsmStatement *stmt,
    ...
    if (stmt->operand_count != 2) {
        fprintf(stderr, "anv-asm: line %d: IMUL requires 2 operands\n", stmt->line);
        return -1;
    }
    const AsmOperand *dst = &stmt->operands[0];
    const AsmOperand *src = &stmt->operands[1];
    if (dst->kind != ASMOP_REG || src->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: IMUL requires reg, reg\n", stmt->line);
        return -1;
    }
...
static int encode_idiv(const AsmStatement *stmt,
    ...
    if (src->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: IDIV requires register\n", stmt->line);
        return -1;
    }
    int W = (src->reg_size == 64);
...
static int encode_unary_f7(const AsmStatement *stmt,
    ...
    const AsmOperand *op = &stmt->operands[0];
    if (op->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: requires register\n", stmt->line);
        return -1;
    }
...
static int encode_incdec(const AsmStatement *stmt,
    ...
    const AsmOperand *op = &stmt->operands[0];
    if (op->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: INC/DEC requires register\n", stmt->line);
        return -1;
    }
    int W = (op->reg_size == 64);
    uint8_t rex = REX(W, 0, 0, REGHI(op->reg_idx));
...
static int encode_shift(const AsmStatement *stmt, uint8_t *buf, int buf_size, int opext) {
    ...
    const AsmOperand *src = &stmt->operands[1];
    if (dst->kind != ASMOP_REG) {
        fprintf(stderr, "anv-asm: line %d: shift requires reg dst\n", stmt->line);
        return -1;
    }
    int W = (dst->reg_size == 64);
    ...
    if (src->kind == ASMOP_IMM) {
        ...
        fprintf(stderr, "anv-asm: line %d: shift src must be imm or cl\n", stmt->line);
        return -1;
    }
    return n;
}
